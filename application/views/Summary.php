 ...
 <Br>
	...
	<?php
		...
		foreach ($results->result() as $res) {
			if ($res->first_char != $current_char) {
				$current_char = $res->first_char;
				echo '<br /><strong><a name = "' . strtoupper($current_char) . '" >' . strtoupper($current_char) . '</a></strong><br />-----<br />';
			}
		$id = base64_encode($res->id);
			echo "<h2 style = 'display:inline; font-size: 14px; text-align: left; font-weight: normal;'><a href = 'detail/" . $prefix . "/" . $res->url . "'>" . $res->name . "</a></h2>";     
		   echo "<br />";
		}  
	?>
</div>